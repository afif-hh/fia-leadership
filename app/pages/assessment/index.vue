...
useHead(() => ({ title: t('assessment.list.title') }))
...
// `locale` is sent explicitly rather than left to the cookie: with `prefix_except_default` the
// reader's language lives in the URL, and the URL is the only source that is certainly right.
// The server falls back to the cookie and then to Accept-Language (`server/http/request-locale.ts`).
const { data, pending, error } = await useFetch<{ versions: TakeableVersion[] }>(
  '/api/v1/assessment/takeable',
  { key: 'assessment-takeable', retry: false, query: { locale } }
)
...
<template>
  <div class="flex flex-col gap-space-6">
    <h1 class="text-ink-900 text-heading-lg font-semibold">{{ t('assessment.list.heading') }}</h1>

    <p v-if="error" class="text-destructive text-body-sm" role="alert">
      {{ t('assessment.list.loadFailed') }}
    </p>
      ...
      >
        <div class="flex flex-col gap-space-1">
          ...
          <p class="text-muted-600 text-body-sm">
            {{ t('assessment.list.versionLine', { version: version.versionNo }) }} ·
            {{ t('assessment.list.itemCount', version.itemCount) }}
          </p>
        </div>
        ...
        <div>
          <Button
            ...
            as="a"
            :href="localePath(`/assessment/${version.versionId}/consent`)"
          >
            {{ t('assessment.list.start') }}
          </Button>
          ...
          <span v-else class="text-success-700 text-body-sm font-semibold">{{
            t('assessment.list.done')
          }}</span>
        </div>
      </li>
    </ul>
