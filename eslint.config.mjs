...
export default withNuxt(
  ...
  },
  {
    ...
    // Generated by the shadcn-vue CLI, not hand-written: every optional prop here
    // (`variant`, `size`, `class`, ...) is deliberately left without a default so the
    // underlying primitive's own default applies. Re-running `shadcn-vue add` to pick
    // up an upstream fix would just regenerate the same shape, so the rule is scoped
    // off here rather than chasing it component by component.
